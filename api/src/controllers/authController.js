...
export async function login(req, res) {
  try {
    ...
    // 🟢 Skapa ny token varje gång användaren loggar in
    ...
    // 🟢 Spara även expires-tid så frontend kan hantera det
    const decoded = jwt.decode(token);

    res.json({
      message: "Inloggning lyckades ",
      token,
      expiresAt: decoded.exp * 1000, // skicka med token-utgångstid
      user: {
        id: user.id,
